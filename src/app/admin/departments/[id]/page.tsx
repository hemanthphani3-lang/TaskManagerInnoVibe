...
export default async function DepartmentBreakdownPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-8">
      <Link href="/admin/dashboard" className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-slate-900 mb-6 transition-colors">
      ...
      <header className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-6">
        ...
        <Link 
          href={`/admin/employees/${departmentId}`}
          className="inline-flex items-center gap-1.5 bg-[#0066FF] hover:bg-[#0052CC] text-white px-4 py-2.5 rounded-xl text-xs font-bold transition-all active:scale-95 shadow-sm shadow-[#0066FF]/20"
        >
          View Department Head Profile
        </Link>
      </header>
