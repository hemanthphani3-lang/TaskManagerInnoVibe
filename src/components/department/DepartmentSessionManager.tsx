...
export function DepartmentSessionManager({ children, links }: DepartmentSessionManagerProps) {
  ...
  useEffect(() => {
    // Listen for backend forceful logout when department/admin approves the request
    ...
    const setupRealtime = async () => {
      ...
      const channelName = `dept_session_${user.id}_${Date.now()}`
      ...
      // Polling fallback: check every 10 seconds for status
