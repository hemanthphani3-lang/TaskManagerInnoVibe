...
export function Sidebar({ title, links, onLogoutClick }: SidebarProps) {
  ...
  useEffect(() => {
    ...
    const setupNotifications = async () => {
      ...
      if (isMounted && count !== null) setUnreadNotifications(count)

      if (!isMounted) return

      // Realtime subscription using a unique channel name to prevent duplicate subscription errors
      const channelName = `sidebar_notifications_${user.id}_${Math.random().toString(36).substring(2, 9)}`
      channel = supabase
        .channel(channelName)
        .on(
          'postgres_changes',
          ...
          () => {
            if (isMounted) setUnreadNotifications(prev => prev + 1)
          }
        )
        .on(
          'postgres_changes',
